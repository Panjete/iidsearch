from readers_01 import getDocs
from math import log2
import heapq

def perQscore(term, postings_file, pointers, docIDsize, tf_length, numdocs, accumulators):
    ...
    docnums_for_this_term, tfs = getDocs(term, pointers, postings_file, docIDsize, tf_length)
    ...
    for i in range(len(docnums_for_this_term)):
        ...
        idfi_term = log2(1 + (numdocs/pointers[term][1])) 
...
def acc_query(terms, postings_file, pointers, docIDsize, tf_length, numdocs):
    accumulators = {}
    for term in terms:
        perQscore(term, postings_file, pointers, docIDsize, tf_length, numdocs, accumulators)
    return accumulators
...
def processQ(terms, postings_file, pointers, docIDsize, tf_length, numdocs, maptoDocs):
    accumulators = acc_query(terms, postings_file, pointers, docIDsize, tf_length, numdocs)
